...
// 7. Chức năng chuyển đổi Tab (ĐÃ SỬA - THÊM KHỞI TẠO BAN ĐẦU)
function setupTabSwitching() {
    // KHỞI TẠO BAN ĐẦU: Ẩn tất cả tab content, chỉ hiển thị tab mặc định
    ...
    // Hiển thị tab content mặc định (description)
    if (defaultTab) {
    ...
    document.querySelectorAll(".tab-button").forEach(button => {
        button.addEventListener("click", () => {
            ...
            // 4. Tìm và hiển thị nội dung tương ứng
...
// 6. CHỨC NĂNG MỚI: Thêm/Xóa Yêu thích từ thẻ sản phẩm (sản phẩm liên quan/đã xem)
    function addToWishlistFromCard(event, productName) {
        // NGĂN CHẶN LAN TRUYỀN (Event Bubbling): Tuyệt đối cần phải có để modal hiện
        event.stopPropagation(); 

        // NGĂN CHẶN HÀNH VI MẶC ĐỊNH (Link): Đảm bảo trình duyệt không nhảy link
        ...
        if (heartIcon.classList.contains('far')) { // Nếu đang là tim rỗng
            ...
            // Sửa lỗi: Modal sẽ hiện ra sau khi thêm event.stopPropagation()
    ...
    // 9. KHỞI TẠO TAB: Gọi hàm khởi tạo tab khi tài liệu đã sẵn sàng
